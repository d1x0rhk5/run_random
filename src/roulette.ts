...
export type RouletteConfig = {
  mount?: HTMLElement;
  cameraMode?: 'dynamic' | 'fixed';
  showBuiltinUi?: boolean;
...
export class Roulette extends EventTarget {
  ...
  private _config: Required<RouletteConfig>;
  ...
  protected createRenderer(): RouletteRenderer {
    return new RouletteRenderer({
      mount: this._config.mount,
      canvasClassName: this._config.canvasClassName,
      showWinnerBanner: this._config.showWinnerBanner,
    });
  }
  ...
  constructor(config: RouletteConfig = {}) {
    ...
    this._config = {
      ...
      cameraMode: config.cameraMode ?? 'dynamic',
      showBuiltinUi: config.showBuiltinUi ?? true,
      canvasClassName: config.canvasClassName ?? 'roulette-canvas',
      showWinnerBanner: config.showWinnerBanner ?? true,
      initialTheme: config.initialTheme ?? 'dark',
    };
    this._theme = Themes[this._config.initialTheme];
  ...
  private async _init() {
    ...
    if (this._config.showBuiltinUi) {
      ...
      this.fastForwarder = { speed: 1 } as FastForwader;
    }
  ...
  public setMarbles(names: string[]) {
    ...
    if (totalCount > 0 && this._config.cameraMode === 'dynamic') {
