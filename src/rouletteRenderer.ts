...
export type RouletteRendererOptions = {
  mount?: HTMLElement;
...
export class RouletteRenderer {
  ...
  protected _canvasClassName: string;
  protected _showWinnerBanner: boolean;

  constructor(options: RouletteRendererOptions = {}) {
    this._keywordService = this.createKeywordService();
    this._mount = options.mount ?? document.body;
    this._canvasClassName = options.canvasClassName ?? 'roulette-canvas';
    this._showWinnerBanner = options.showWinnerBanner ?? true;
  }
  ...
  async init() {
    ...
    this._canvas.className = this._canvasClassName;
